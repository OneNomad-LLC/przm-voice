 ...
 * we're flagging.
 */
export function detectFlattery(assistantText) {
    ...
    if (matchesAny(assistantText, EVIDENCE_MARKERS))
        return null;
 ...
 */
export function detectWalkBack(currentAssistantText, priorAssistantText, intermediateUserText) {
    if (!matchesAny(currentAssistantText, RETRACTION_PATTERNS))
        return null;
    const currentHasEvidence = matchesAny(currentAssistantText, EVIDENCE_MARKERS);
    if (currentHasEvidence)
        return null; // grounded retraction
    const userHasEvidence = intermediateUserText
        ? matchesAny(intermediateUserText, USER_EVIDENCE_MARKERS)
        : false;
    if (userHasEvidence)
        return null; // user supplied facts; reasonable update
 ...
 */
export function detectPositionFlip(currentAssistantText, priorAssistantText, intermediateUserText) {
    if (!currentAssistantText || !priorAssistantText)
        return null;
    // Bail when evidence is present on either side.
    if (matchesAny(currentAssistantText, EVIDENCE_MARKERS))
        return null;
    if (matchesAny(intermediateUserText, USER_EVIDENCE_MARKERS))
        return null;
    // Find shared significant words (length > 4) between the two
    // assistant turns — the anchor. If no anchor, this isn't talking
    // about the same thing.
    const priorWords = new Set(priorAssistantText.toLowerCase().split(/\W+/).filter((w) => w.length > 4));
    const currentWords = new Set(currentAssistantText.toLowerCase().split(/\W+/).filter((w) => w.length > 4));
    const shared = [];
    for (const w of priorWords)
        if (currentWords.has(w))
            shared.push(w);
    if (shared.length < 1)
        return null; // need at least one shared significant token
    ...
    if (!flippedPosToNeg && !flippedNegToPos)
        return null;
 ...
 */
export function detectAgreementCascade(recentAssistantTurns, threshold = 4) {
    if (recentAssistantTurns.length < threshold)
        return null;
    ...
    for (let i = recentAssistantTurns.length - 1; i >= 0; i--) {
        ...
        if (matchesAny(turn, DISAGREEMENT_MARKERS))
            break;
        consecutive++;
    }
...
export function detectSycophancyInAssistant(input) {
    ...
    const walkBack = detectWalkBack(input.currentAssistantText, input.priorAssistantText, input.intermediateUserText);
    if (walkBack)
        out.push(walkBack);
    if (input.priorAssistantText && input.intermediateUserText) {
        const flip = detectPositionFlip(input.currentAssistantText, input.priorAssistantText, input.intermediateUserText);
        if (flip)
            out.push(flip);
    }
    if (input.recentAssistantTurns && input.recentAssistantTurns.length > 0) {
        const cascade = detectAgreementCascade(input.recentAssistantTurns, input.cascadeThreshold);
        if (cascade)
            out.push(cascade);
    }
 ...
 */
export function toSignalType(t) {
    return t;
}
