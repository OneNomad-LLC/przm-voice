...
export type PollResult =
  | { kind: 'approved'; api_url: string; api_key: string; label: string | null; scopes: string[] }
  | { kind: 'denied' }
  | { kind: 'expired' }
  | { kind: 'timeout' }
...
export interface PollUntilTerminalOpts {
  ...
  expiresInMs: number;
  fetch: typeof fetch;
  // Injectable so tests can use fake timers / immediate resolution.
 ...
 */
export async function pollUntilTerminal(opts: PollUntilTerminalOpts): Promise<PollResult> {
...
async function pollOnceWithRetry(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      lastErrMessage = (err as Error).message || String(err);
      await sleep(200 * Math.pow(2, attempt));
    }
  }
  return {
...
function tryOpenBrowser(url: string): void {
  try {
    if (process.platform === 'darwin') {
      ...
      spawn('cmd', ['/c', 'start', '', url], { stdio: 'ignore', detached: true, shell: false }).on(
        'error',
        () => {},
      );
    } else {
...
export function resolveServerUrl(args: LoginArgs): string | null {
  if (args.serverUrl && args.serverUrl.length > 0) return args.serverUrl;
  if (args.serverFlag && args.serverFlag.length > 0) return args.serverFlag;
...
export async function runLogin(argv: string[]): Promise<void> {
  ...
  if (!serverUrl) {
    process.stderr.write(
      'Server URL required. Pass it as an argument, --server <url>, or set PYRE_API_URL.\n',
    );
    process.exit(1);
  }
  ...
  try {
    ...
    process.stderr.write(
      `Network error talking to ${normalizedServer}: ${(err as Error).message}\n`,
    );
    process.exit(1);
  }
  ...
  if (!initRes.ok) {
    ...
    process.stderr.write(
      `Network error talking to ${normalizedServer}: HTTP ${initRes.status} ${text.slice(0, 200)}\n`,
    );
    process.exit(1);
  }

  const code = (await initRes.json()) as DeviceCodeResponse;
