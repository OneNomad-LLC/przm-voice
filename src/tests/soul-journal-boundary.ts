...
async function run(): Promise<void> {
  ...
  try {
    ...
    assert(
      personalityAfterSynth === SOUL_SENTINEL,
      `soul/personality was modified by updateSoulFromSynthesis!\n` +
    ...
    assert(
      styleAfterSynth === '' || !styleAfterSynth.includes('Communication Style\n\n## Length'),
      `soul/style was written by synthesis — boundary violated!\n` +
    ...
    if (result.updated) {
      ...
      assert(
        journalPersonality.length > 0,
        'journal/personality is empty but synthesis reported updates',
      );
      assert(
        journalPersonality.includes('<!-- synthesis:'),
        `journal/personality missing synthesis marker:\n  got: ${JSON.stringify(journalPersonality.slice(0, 200))}`,
      );
      ...
      if (result.changes.some(c => c.includes('style'))) {
        ...
        assert(
          journalStyle.length > 0,
          'journal/style is empty but synthesis reported style changes',
        );
        assert(
          journalStyle.includes('<!-- synthesis:'),
          `journal/style missing synthesis marker:\n  got: ${JSON.stringify(journalStyle.slice(0, 200))}`,
        );
    ...
    if (!presetResult.applied) {
      ...
      assert(
        personalityAfterPreset === SOUL_SENTINEL,
        `soul/personality was modified by applySoulPreset!\n` +
      ...
      assert(
        journalAfterPreset.includes('<!-- preset:default:'),
        `journal/personality missing preset marker after applySoulPreset:\n` +
