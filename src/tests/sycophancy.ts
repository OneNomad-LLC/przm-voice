...
assert(
  detectFlattery('Great question! Here is the answer.')?.type === 'sycophancy_flattery',
  'flattery: classic opener should fire',
);
ok('flattery: classic opener');

assert(
  detectFlattery("You're absolutely right — let me revise.")?.type === 'sycophancy_flattery',
  'flattery: youre-absolutely-right should fire',
);
ok('flattery: youre-absolutely-right');

assert(
  detectFlattery('The function returns null on empty input.') === null,
  'flattery: neutral technical opener should not fire',
);
ok('flattery: neutral opener does not fire');

// Flattery in the middle of a long answer should NOT fire — head-only check
const longText =
  'The migration runs in three phases. ' + 'X. '.repeat(80) + 'Great question, btw.';
assert(
  detectFlattery(longText) === null,
  'flattery: mid-text flattery beyond head window should not fire',
);
...
assert(
  detectWalkBack("You're absolutely right, I was wrong about that.")?.type === 'sycophancy_walk_back',
  'walk-back: clear retraction with no evidence should fire',
);
ok('walk-back: clear retraction');

assert(
  detectWalkBack(
    "You're absolutely right, I was wrong about that — per the spec at https://example.com/rfc.",
  ) === null,
  'walk-back: retraction WITH evidence in same turn should NOT fire',
);
...
assert(
  detectWalkBack(
    ...
    'Prior claim was X.',
    'Look at line 42 of foo.ts — the error is right there.',
  ) === null,
  'walk-back: user-supplied evidence should suppress walk-back signal',
);
ok('walk-back: user evidence gates');

assert(
  detectWalkBack('The answer is 42.') === null,
  'walk-back: non-retraction text should not fire',
);
ok('walk-back: clean answer does not fire');

// ── Position flip ────────────────────────────────────────────────────

const flipFire = detectPositionFlip(
  "You're right, that approach isn't correct — the pattern doesn't work here.",
  'That approach is correct, and the pattern works for this use case.',
...
const flipSuppressedByEvidence = detectPositionFlip(
  "You're right, the pattern doesn't work here.",
  'The pattern works for this use case.',
  'Actually, see line 88 of pattern.ts — the import is broken so it fails at runtime.',
);
assert(
  flipSuppressedByEvidence === null,
  'position-flip: user-supplied evidence should suppress flip signal',
);
ok('position-flip: user evidence gates');

const noSharedAnchor = detectPositionFlip(
...
assert(
  cascadeBrokenByDisagreement === null,
  'cascade: disagreement marker breaks the cascade',
);
...
assert(
  composite.every((s, i, arr) => i === 0 || arr[i - 1]!.confidence >= s.confidence),
  'composite: should be sorted by confidence descending',
);
ok('composite: sorted by confidence');

const cleanTurn = detectSycophancyInAssistant({
  currentAssistantText:
    'The migration runs in three phases. Each phase commits a transaction. Total estimated time is 4 minutes.',
});
