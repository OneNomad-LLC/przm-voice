...
function withTempCredsDir<T>(fn: (path: string) => Promise<T> | T): Promise<T> {
  ...
  return Promise.resolve(fn(file)).finally(() => {
    if (prev === undefined) delete process.env.PYRE_CREDENTIALS_FILE;
    else process.env.PYRE_CREDENTIALS_FILE = prev;
    rmSync(dir, { recursive: true, force: true });
  });
}

function clearStorageEnv(): { restore: () => void } {
...
async function testCredentialsRoundTrip(): Promise<void> {
...
async function testRejectsMalformed(): Promise<void> {
  await withTempCredsDir(async (path) => {
...
async function testResolverPicksFile(): Promise<void> {
...
async function testResolverPicksCloudFromCreds(): Promise<void> {
  const env = clearStorageEnv();
  try {
    await withTempCredsDir(async () => {
...
async function testEnvOverridesCredsFields(): Promise<void> {
  const env = clearStorageEnv();
  try {
    await withTempCredsDir(async () => {
      await writeCredentials({
      ...
      globalThis.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (u.includes('/api/persona/profile')) seenUrl = u;
      ...
      try {
        ...
        assert(
          seenUrl.startsWith('https://from-env.example.com'),
          `PERSONA_API_URL should override creds (saw ${seenUrl})`,
        );
      } finally {
        globalThis.fetch = originalFetch;
      }
...
async function testStorageBackendForceFile(): Promise<void> {
  const env = clearStorageEnv();
  const dataDir = mkdtempSync(join(tmpdir(), 'persona-auth-data-'));
  process.env.PERSONA_DATA_DIR = dataDir;
  ...
  try {
    await withTempCredsDir(async () => {
      ...
      assert(
        ...
        'STORAGE_BACKEND=file forces file even with creds present',
      );
    });
  } finally {
    env.restore();
...
async function testPollHappyPath(): Promise<void> {
  let calls = 0;
  ...
  if (result.kind === 'approved') {
    assert(
      result.api_url === 'https://pyre-runtime.example.com',
      'approved api_url passes through',
    );
...
async function testPollDenied(): Promise<void> {
  const mockFetch: typeof fetch = (async () =>
...
async function testPollExpiredViaHttp410(): Promise<void> {
  const mockFetch: typeof fetch = (async () =>
...
async function testPollTimeout(): Promise<void> {
  const mockFetch: typeof fetch = (async () =>
...
async function testPollNetworkRetryThenSurface(): Promise<void> {
  let calls = 0;
  const mockFetch: typeof fetch = (async () => {
    calls++;
