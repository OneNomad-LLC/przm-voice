...
async function runSmoke(adapter: StorageAdapter): Promise<void> {
  setStorage(adapter);
  ...
  assert(
    signals.some((s) => s.id === signal.id),
    'newly recorded signal must appear in listSignals',
  );
